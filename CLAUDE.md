...
**一条规矩放哪，看谁读得到、它该不该变。** 上面那句判断方法还缺中间一层：

| 放哪       | 放什么                                                           | 例子                                   |
| ---------- | ---------------------------------------------------------------- | -------------------------------------- |
| 全局提示词 | 四个阶段都成立、永远不变的事实                                   | A 股交易制度                           |
| 阶段提示词 | 这个阶段的职责，以及它和系统之间的协议：必须调哪些工具、报告怎么写 | 每次都声明敞口上限、交易完登记挂单计划 |
| 长期记忆   | 什么情况下该怎么做——会被新证据推翻的                             | 止损执行口径、挂单深度、平台受理的挂单偏离 |

硬规则不能挪进记忆，哪怕它看起来只是「常识」：

- **Watch 和 Summary 读不到记忆。** `memory_read`／`memory_write` 的 `enabled_stages` 都是
  `("Run",)`，Dream 用的是自己那套整理工具。真正撤单的是 Watch，「14:57 之后不能撤单」放进
  记忆，它永远看不见。
- **记忆要检索才出现，提示词一直在眼前。** 2026-09-24 那笔卖 150 股：下单工具的说明写着
  「数量必须为 100 的整数倍」，模型照的却是当时全局里写错的「卖出可以不足 100 股」——
  动手那一刻，它照的是提示词。
- **记忆是 agent 自己改的地方。** Run 写，Dream 合并、改写、删除；经验该随证据变，事实不该。
  20% 的敞口上限就是住进记忆之后，被六条经验层层继承，成了「事实」。

阶段协议同样不能进记忆，因为代码依赖它：Watch 靠读 Run 登记的挂单计划干活，Dream 哪晚把
「交易完要登记」合并掉，流水线会悄悄断掉，不报错。

身份描述也要过「四个阶段都成立」这一关。全局写成「独立完成研究、决策与复盘」，Watch 读到
的开头就和它自己的「不做研究，不形成新的看法」打架。
