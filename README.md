...
| **记忆梦境**       | 保存可复用的交易经验，记录每一次读取/写入/修改，夜间自动整理并保留经验的凝练来源。          |
| **阶段与模型配置** | 为不同运行阶段配置提示词、模型渠道、模型参数和选择策略。                                    |
| **任务调度**       | 配置交易时段内的自动运行计划，以及账户刷新和记忆整理任务。                                  |
| **推送通知**       | 下单、撤单、成交、运行完成与失败时推送到 Webhook、Server酱、企业微信或 macOS 桌面。         |
| **报告邮件**       | 把运行报告的 HTML 页面通过 Resend 投递到邮箱；开启离开模式后每次运行结束自动发送。          |
| **关注清单**       | 记录你自己关注的公司，运行时作为额外参考送达，不改变智能体自身的选股过程。                  |
...
运行本身是 **Run → Summary** 两个阶段：Run 是唯一能调用工具、能操作账户的阶段，研究、判断、交易都在它内部完成；Summary 只把报告渲染成 HTML。`Dream` 是独立的记忆整理任务，不属于这条流水线——它在收盘之后到次日开盘之前的某个时刻执行（可配置），每次挑最近几个有运行、却还没整理过的交易日，所以错过一晚不会永久遗漏。
