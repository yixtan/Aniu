...
const markdownComponents = {
  ...
  table: ({ children }: { children?: ReactNode }) => (
    <div className="my-3 overflow-x-auto rounded-md border border-sky-100">
  ...
  thead: ({ children }: { children?: ReactNode }) => (
    <thead className="bg-sky-50 text-sky-900">{children}</thead>
  ),
  tbody: ({ children }: { children?: ReactNode }) => (
    <tbody className="[&_tr:last-child_td]:border-b-0">{children}</tbody>
  ),
  tr: ({ children }: { children?: ReactNode }) => (
    <tr className="transition-colors even:bg-slate-50/70 hover:bg-sky-50/50">{children}</tr>
  ),
  th: ({ children }: { children?: ReactNode }) => (
    <th className="border-b border-sky-200 px-2 py-1.5 text-start font-semibold whitespace-nowrap">
      {children}
    </th>
  ),
  td: ({ children }: { children?: ReactNode }) => (
    <td className="first:text-foreground border-b border-slate-100 px-2 py-1.5 align-top first:font-medium">
      {children}
    </td>
  ),
};
